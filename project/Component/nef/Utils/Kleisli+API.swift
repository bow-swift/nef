...
public extension Kleisli where D == NefModels.Console, F == IOPartial<nef.Error> {
    ...
    /// Performs the side effects that are suspended in this IO in a synchronous manner. Resolve `D` dependency of type `Console` using a dummy console.
    ...
    /// Performs the side effects that are suspended in this IO in an asynchronous manner. Resolve `D` dependency of type `Console` using a dummy console.
    /// - Parameters
